...
for i in range(1,11):
  ...
  opts = ['train','test']
  for opt in opts:
      df = pd.read_csv(f'data/data_HLM/fold-{i}/' + opt + '.csv')
      # print(df)
  ...
  processed_train = f'data/data_HLM/processed/fold-{i}/' + 'train.pt'
  processed_test = f'data/data_HLM/processed/fold-{i}/' + 'test.pt'
  if ((not os.path.isfile(processed_train)) or (not os.path.isfile(processed_test))):

      df = pd.read_csv(f'data/data_HLM/fold-{i}/' + 'train.csv')
      train_compounds = list(df['SMILES'])
      train_compounds = np.asarray(train_compounds)
      train_Y = np.array(pd.DataFrame(df['Label'])).tolist()

      #text embedding
      file_name = f'processed_data/HLM_text_embedding.json'
      total_smile=[]
      total_embedding = []
      train_embedding=[]
      ...
      df = pd.read_csv(f'data/data_HLM/fold-{i}/' + 'test.csv')
      ...
      train_data = TestbedDataset(root=f'data/data_HLM/fold-{i}/', dataset='train', xd=train_compounds, y=train_Y,
                                  smile_graph=smile_graph, text_embedding=train_embedding)
      print('preparing,' + 'test.pt in pytorch format!')
      test_data = TestbedDataset(root=f'data/data_HLM/fold-{i}/', dataset= 'test', xd=test_compounds, y=test_Y,
